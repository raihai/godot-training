...
public partial class JumpState : BaseState
{
    ...
    public override void OnEnterState() {
        ...
        test.Y = 10f;
        player.PlayerBody3D.Velocity = test;
    ...
    public override void PhysicsUpdate(double delta) {
        ...
        if (!player.PlayerBody3D.IsOnFloor()) {
            player.PlayerBody3D.Velocity += Vector3.Down * 15 * (float)delta;
            GD.Print(player.PlayerBody3D.Velocity);
            
        }

        player.PlayerBody3D.FloorSnapLength = player.PlayerBody3D.IsOnFloor() ? 0.2f : 0f;
